<template>
  ...
  <div>
  <form action="" style="padding-block: 6rem;">
    <!-- <select class="block  px-5 py-2 rounded rounded-lg bg-light" v-model="roleName" @change="Getpermissions" name="Role Name" id="">
    <option  value="">Select Role</option>
    <option v-for="(item,index) in userRoles" :key="index" :value="[item.name,item.id]">{{item.name}}</option>

    </select> -->
    <div class="mb-3">
            <select v-model="roleName" @change="Getpermissions" class="w-25 form-select form-select-lg mb-3" aria-label=".form-select-lg example">
                <option value="">Select Role</option>
                <option :value="[item.name,item.id]" v-for="(item,index) in userRoles" :key="index">{{ item.name }}</option>
  ...
  <h5>Permissions:</h5>
  <hr>
  <div v-if="!roleName"><p>Please select a role to view associated permissions</p></div>
<div v-else class="d-flex justify-content-around">
<div class="row">
  <b-col lg="3"  v-for="(items,index) in results" :key="index">
          ...
          <b-card-body>
            <div   class="py-1">
              <div v-for="(item,index) in permissions " :key="index"  >
                <template v-if="item.module == items.id">
    <div class="d-flex justify-content-start align-items-center gap-2 my-2" >
      <input class="form-check-input" :value="item.id" :id="item.id" @click="updatePermissions(item.id)" type="checkbox" :checked="response.find(i=> i == item.name)"   aria-label="...">
      <label style="font-size: 20px;" :for='item.id'>{{item.name}}</label>
    </div>
              </template>
              </div>
  </div>
...
<button v-if="roleName" @click="submitPermissions" class="btn btn-primary">Submit</button>
  </div>
  <!-- <b-row>
    <b-col sm="12">
...
import { allRoles,getModules,getPermissions, updaterolepermissions } from '@/service';
...
export default {
  setup(){
    ...
    onMounted(async()=>{
      try{ 
       await allRoles().then(data => userRoles.value = data)
     
        
    }catch(err){
      toast.error('Network Error Roles not available !',{autoclose: 2000})
    
    }})

    onMounted(async()=>{
      try{ await getPermissions().then(data => {permissions.value = data
      })}catch(err){
        toast.error('Network Error Permissions not available !',{autoclose: 2000})
      }
    })
    onMounted(async()=>{
      try{ await getModules().then(data=> {
        if(data.detail){
        ...
        toast.error('Network Error Modules not available !',{autoclose: 2000})
      }
    })
    function Getpermissions (){
      const findvalue = userRoles.value
      let result = []
     result.push(findvalue.find(user => user.name == roleName.value[0]))
     allPermissions.value = result?.map(item => item?.modules.map(value=> value.permissions))
      const manipulate = allPermissions?.value[0]
     if(manipulate){
      const arr = []
      for(let x = 0 ; x<manipulate.length ; x++){
        arr.push(...manipulate[x]) 
      }
     allPermissions.value = arr

      response.value = allPermissions.value.map(data =>data.name)
      permissionsId.value.push(allPermissions.value.map(v=> v.id)) 
     }
     else{
      response.value = []
     }
    ...
    function updatePermissions (value) {
      if(permissionsId.value[0].includes(value)){
        let index = permissionsId.value[0].indexOf(value);
        if (index !== -1) {
        permissionsId.value[0].splice(index, 1);
        }
      }else{
        permissionsId.value[0].push(value)
      }
    }
    ...
    async function submitPermissions () {
      const payload = {}
      payload.name = roleName.value[0]
      const roleid = roleName.value[1]
      payload.permissions = permissionsId.value[0]
      try{
        await updaterolepermissions(roleid,payload).then(data => {if(data.message){
